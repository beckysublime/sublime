	...
	onLoad: function () {
		for (let i = 0; i < sciences.length; i++) {
			...
			<div class="basicDiv" id="` + id + `Div">
				<button class="specialButton" onclick="if (` + sciences[i].startRequirement + ` && gameData.` + id + `Bar == 0) {runScienceBar(` + i + `)}">` + sciences[i].buttonText + `</button>
...
sciences = [
	{
		...
		startRequirement: 'gameData.peeledLimesPerJuice > 1'
	},
	{
		id: 'surveying',
		...
		startRequirement: 'gameData.numberOfTiles < 20'
	},
	{
		id: 'benevolence',
