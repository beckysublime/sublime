...
basejs = {
	...
	update: function () {
		...
		checkShow(gameData.maps > 0 && !gameData.pinUnlock, 'pinUnlockDiv')	
		...
		checkShow(!gameData.forestWell, 'buyAWell') //I want this to be seen at the beginning, only to be unlocked FAR later :)
	...
	onLoad: function () {
		...
		checkShow(gameData.lookAround > 2, 'toggleActionsButton', 'inline')
...
function tabMarket(tabby) {
	...
	tabManager('travel')
	tabManager('trade', '#FDFF9A')


	document.getElementById(tabby).style.display = "block"
	
	
	checkColor(tabby == 'trade', tabby + 'Button', '#FCFF4E', '#898989')
